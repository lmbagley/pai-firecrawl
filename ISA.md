...
- **build/release (ISC-43–53):** `bunx tsc --noEmit` clean; `bun test` 27/27, zero live network (fixtures + mocked fetch); README documents install/commands/key/self-host; LICENSE MIT; `.gitignore` excludes keys/.env/node_modules; no `/home/`,`/Users/`,`~/.claude` literal in source (grep 0); CLI PAI-agnostic (env or generic XDG key path); committed `969c951`, **published PUBLIC to github.com/lmbagley/pai-firecrawl 2026-06-15, v0.1.0 tagged + Released** (commit `8a751df`). Community-release polish: README badges, CONTRIBUTING.md, SECURITY.md (private vuln reporting enabled), CHANGELOG.md, package.json metadata, GitHub description + topics. MIT detected; full-history + new-files secret-scrubs clean.
